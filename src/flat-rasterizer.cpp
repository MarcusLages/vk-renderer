...
namespace vkr {
    ...
    // Renders the z-buffer in a gray scale (the furthest, the brighest)
    void FlatRasterizer::draw_triangle_depth(
            mvmath::vec3 a,
            mvmath::vec3 b,
            mvmath::vec3 c,
            IFrameBuffer &fb
        ...
        for(float x = min_box.x; x <= max_box.x; x++) {
            for(float y = min_box.y; y <= max_box.y; y++) {
                ...
                const uint8_t intensity = static_cast<uint8_t>(z * Color::MAX_COLOR_CHANNEL);
                Color col = {intensity, intensity, intensity, intensity};
                fb.set(x, y, z, col);
            }
        }
    }
    ...
    // Renders the z-buffer in a gray scale (the furthest, the brighest)
    void FlatRasterizer::render_depth() {
        std::tuple<int, int> frame = { fb.width() - 1, fb. height() - 1 };
        for(int i = 0; i < model.faces_len(); i++) {
            mvmath::vec3 a = mvmath::central_ortho_project_z(model.vert(i, 0), model.vert_range, frame);
            mvmath::vec3 b = mvmath::central_ortho_project_z(model.vert(i, 1), model.vert_range, frame);
